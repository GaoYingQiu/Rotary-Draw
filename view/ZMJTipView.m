...
- (CGRect)computeFrameWithArrowPosition:(ZMJArrowPosition)postion refViewFrame:(CGRect)refViewFrame superviewFrame:(CGRect)superviewFrame {
    ...
    switch (postion) {
        ...
        case ZMJArrowPosition_bottom:
            xOrigin = 60; // CGRectGetMaxX(refViewFrame) - refViewFrame.size.width / 2 - self.contentSize.width / 2;
            ...
            delegate:(id<ZMJTipViewDelegate>)delegate
{
    ...
    UIView *findView = [superview viewWithTag:1977];
    if(findView){
    ...
    [NSObject jk_performBlock:^{
        [tipview dismissWithCompletion:nil];
    } afterDelay:2];
}

-(void)hideDelayed:(ZMJTipView *)tipView
{
     [self dismissWithCompletion:nil];
}
